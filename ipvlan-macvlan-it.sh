#!/bin/sh
# 
# Dirty IT tests for debugging Macvlan/Ipvlan Docker drivers
#
# View the = command that throws an error.
#set -o xtrace
# Change eth0 to the link to be used for the `-o host_iface=`
ETH=${ETH:-eth0}
EXPECTED_NETWORKS=31
...
verify_results() {
    ...
    NETWORK_COUNT=$(docker network ls | grep "macvlan\|ipvlan" | wc -l)
    echo "#########################################################################################"
    # The results should be 31 networks created
    if [ ${NETWORK_COUNT} -eq 31 ]
    then
        echo "PASS ---------> Macvlan/Ipvlan Networks Created the proper number of networks:[ ${NETWORK_COUNT} ]"
        echo "PASS ---------> Macvlan/Ipvlan Number of Links Exist:[ ${LINK_COUNT} ] ('ip link')"
    else
        echo "FAILED ---------> Macvlan/Ipvlan Networks Created the wrong number of networks:[ ${NETWORK_COUNT} ] should have been [ $EXPECTED_NETWORKS ]"
